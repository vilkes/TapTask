window.addEventListener('load', () => {
  ...
  function validarContrasena() {
    ...
    if (!/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{8,}$/.test(val)) {
      mostrarError(inputContrasena, 'La contrasena debe tener al menos 8 caracteres, una mayúscula, una minúscula, un número y un símbolo.');
      return false;
    } else {
  ...
  function validarFechaNacimiento() {
    ...
    if (fecha > hoy) {
      ...
      mostrarError(inputFecha, 'Debes tener al menos 16 anos.');
      return false;
    } else if (fecha < fechaMinima) {
      mostrarError(inputFecha, 'La edad máxima permitida es 100 anos.');
      return false;
    } else {
