...
const formularios = document.getElementById("form");
const fotoPerfil = document.getElementById("file-input"); // tu input file
const resena = document.getElementById("opinion"); // tu textarea

if(resena){
  resena.addEventListener("input", (e) => {
    let valor = e.target.value;
    if (valor.length > 1800) {
      valor = valor.slice(0, 1800);
      alert(`Ya has escrito ${valor.length} caracteres, el máximo es 1800.`);
    }
  ...
  fechaNacimiento.addEventListener("input", (e) => {
    ...
    if (partes.length === 3) {
      ...
      if (edad < 18 || (edad === 18 && (month > (hoy.getMonth() + 1) || (month === (hoy.getMonth() + 1) && day > hoy.getDate())))) {
        alert("Debes tener al menos 18 anos.");
        e.target.value = "";
      }
    }
  });
